...
__dumb__ DumbBasedShader::DumbBasedShader(const ColorRGB &fresnelFactor, float cpecular) {
	fres = fresnelFactor;
	spec = cpecular;
}
...
__dumb__ Color DumbBasedShader::getReflectedColor(const ShaderReflectedColorRequest<BakedTriFace> &request)const {
	...
	Color brdf(max(0.0f, min(brdfBare.r, 1.0f)), max(0.0f, min(brdfBare.g, 1.0f)), max(0.0f, min(brdfBare.b, 1.0f)));
	
	return brdf * request.photon.color;
}
